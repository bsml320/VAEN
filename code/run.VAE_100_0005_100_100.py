...
val_file_2_path = sys.argv[5] #GTEx.zeroone_5000_0.2.tsv
train_latent_file = sys.argv[6] #CCLE_latent_5000_0.2.tsv
train_weight_file = sys.argv[7] #CCLE_gene_weights_5000_0.2.tsv
val_1_pred_file = sys.argv[8] #PANCAN_prediction_5000_0.2.tsv
val_2_pred_file = sys.argv[9] #PANCAN_prediction_5000_0.2.tsv
encoder_file = sys.argv[10] # CCLE_encoder_onehidden_vae.hdf5
decoder_file = sys.argv[11] # CCLE_decoder_onehidden_vae.hdf5
...
####val_file = os.path.join('/data1_2/jiap/projects/18-CCLE-VAE/SCALE', 'PANCAN.zeroone_5000_0.2.tsv')
val_file_2 = os.path.join(work_dir, val_file_2_path)
val_df_2 = pd.read_table(val_file_2, index_col=0)
...
###################################
encoded_val_df = encoder.predict_on_batch(val_df_2)
encoded_val_df = pd.DataFrame(encoded_val_df, index=val_df_2.index)

encoded_val_df.columns.name = 'sample_id'
encoded_val_df.columns = encoded_val_df.columns + 1
####encoded_file = os.path.join('/data1_2/jiap/projects/18-CCLE-VAE/SCALE', 'PANCAN_prediction_5000_0.2.tsv')
encoded_file = os.path.join(work_dir+"/result/", sys.argv[1]+"."+val_2_pred_file)
encoded_val_df.to_csv(encoded_file, sep='\t')
encoded_val_df.head(2)
