...
setwd("/work/result.EN/dr.GDSC/01/")
...
cell.line.anno = read.csv("/work/data/GDSC//DepMap-2018q3-celllines.csv", as.is=T)

for(ksigmoid.1 in start:end){
	
	ksigmoid = ksigmoid.1 * scale.factor
	...
	TCGA.pred = read.table(paste("/work/result/", ksigmoid, ".TCGA.latent.tsv", sep=""), header=T, sep="\t", as.is=T)
	TCGA.test.data = TCGA.pred[,-1]
	
	########### Prediction
	PPs = read.table(paste("/work/result/",ksigmoid,".CCLE.latent.tsv", sep=""))
	...
	for(k in 1:length(drugs)){
		...
		for(kk in 1:10){
			...
			SCC = c(SCC, cor(Y, res.list$self_pred[,1], method="spearman"))
