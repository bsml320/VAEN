...
########################
### RUN /work/result.EN/dr.GDSC/04-mix/12.TML/04.12.GDSC.TumorMutationLoad.R
########################


dat.mat = read.table("/work/result.EN/dr.GDSC/04-all/12.TML/GDSC.cancer.drug.tml.wilcox.txt", header=T, as.is=T)
dat.mat$adjp = p.adjust(dat.mat[,3], method="BH")
...
for(k in 1:length(ii)){
	i = ii[k]
	if(k==7) {
		text(log2(dat.mat[i, 4]), -log10(dat.mat[i, 3]), labels=dat.mat[i, 2], pos=1, cex=.9)
	} else if(k == 4 | k==6){ ### "Paclitaxel", "JW.7.52.1"
		text(log2(dat.mat[i, 4]), -log10(dat.mat[i, 3]), labels=dat.mat[i, 2], pos=4, cex=.9)
	} else {
		text(log2(dat.mat[i, 4]), -log10(dat.mat[i, 3]), labels=dat.mat[i, 2], pos=2, cex=.9)
	}
}
...
gdsc.anno = read.delim("/work/data/GDSC/Screened_Compounds-2.txt", as.is=T)
match(dat.mat[,2], gdsc.anno[,2]) -> ii
cbind(dat.mat, gdsc.anno[ii,2], gdsc.anno[ii, ]) -> new.mat


pdf("Figure5G.GDSC.enrich.OR.right-wilcox.pdf", width=3.6, height=4)
...
for(k in 1:length(tag)){
	...
	x = ifelse(log2(dat.mat[,4]) > 0.2 & dat.mat[,5] < 0.05, 0, 1 )
...
########### plot2
pdf("Figure5F.GDSC.enrich.OR.left-wilcox.pdf", width=4, height=4)
par(mfrow=c(1, 1), mar=c(0.2,3,1,1))
par(lheight=.8)
