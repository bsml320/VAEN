...
### RUN 03.01.CCLE.lineage.pre-3BCD.R

load("/work/result.EN/dr.CCLE/03.lineage/CCLE.ActArea.lineage.RData")

pdf("3C.CCLE.lineage.pdf", width=5.7, height=5)
TCGA.cutoff = 0.05/(33*24)
TCGA.cutoff = 0.05
par(mar=c(1,6,10,4))
...
p <- ggplot(dat, aes(x = Cancer, y = Prop, group = grp, fill = grp)) +
  ...
  scale_y_continuous(breaks = seq(-0.4, 1, 0.1), labels = abs(seq(-0.4, 1, 0.1))) +
