...
library(ggplot2)
library(magrittr)
source("/path/to/VAEN/code/multiplot.R")

one.drugs.match = read.table("/path/to/VAEN/DATA/drugs.match.txt", as.is=T)
two.drugs.match = read.table("/path/to/VAEN/DATA/drugs.match-2.txt", as.is=T, sep="\t")

cbind(one.drugs.match, two.drugs.match[,-2]) -> drugs.match
write.table(drugs.match, file="drugs.name.match.txt", row.names=F, quote=F, sep="\t", col.names=F)


ccle.anno = read.csv("/path/to/VAEN/DATA/CCLE/CCLE_NP24.2009_Drug_data_2015.02.24.csv", as.is=T)
gdsc.anno = read.delim("/path/to/VAEN/DATA/GDSC/v17.3_fitted_dose_response.txt", as.is=T)
cell.line.anno = read.csv("/path/to/VAEN/DATA/CCLE/DepMap-2018q3-celllines.csv", as.is=T)

PPs = read.table(paste("/path/to/VAEN/result/1.CCLE.latent.tsv", sep=""))
...
ccle.pred = read.table("/path/to/VAEN/result.EN/dr.CCLE/VAEN_CCLE.A.pred_CCLE.txt", header=T, as.is=T)
gdsc.pred = read.table("/path/to/VAEN/result.EN/dr.GDSC/VAEN_GDSC.A.pred_GDSC.txt", header=T, as.is=T, sep="\t")
...
write.table(shared.drugs.pred.cor, file="A.best.shared.drugs.pred.cor.txt", row.names=F, quote=F, sep="\t")
write.table(shared.drugs.ori.cor, file="A.best.shared.drugs.ori.cor.txt", row.names=F, quote=F, sep="\t")
...
for(k in 1:nrow(two.drugs.match)){
	...
	#################### 3. GDSC predicted
	load(paste("/path/to/VAEN/result.EN/dr.GDSC/01/1.GDSC.model.list.RData", sep="") )
...
pdf("NOPEER.RANK.Sigmoid.A.best.prediction.shared.drugs.pdf", width=21, height=15)
...
melt(new) -> dat
pdf("2E.prediction.shared.drugs.boxplot.pdf", width=4, height=4) 
