...
for(ksigmoid in 1:100){
	fn = paste("/path/to/GitHub/result.EN/dr.CCLE/01S/", ksigmoid, ".CCLE.model.list.S.RData", sep="")
	if(!file.exists(fn))next
	load(paste("/path/to/GitHub/result.EN/dr.CCLE/01S/", ksigmoid, ".CCLE.model.list.S.RData", sep=""))
...
write.table(holdout.R2, file="CCLE.S.holdoutR2.txt", quote=F, sep="\t", row.names=FALSE)
...
for(k in 1:length(drugs)){
	...
	load( paste("/path/to/GitHub/result.EN/dr.CCLE/01S/", best.index ,".CCLE.model.list.S.RData", sep="") )
