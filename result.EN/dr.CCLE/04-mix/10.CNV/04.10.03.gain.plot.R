...
for(kf in 1:length(files)){
	...
	gain.sig.mat = read.table(paste("gain.full/",files[kf],sep=""), as.is=T, sep="\t")
...
tiff("manhantan.gain.sensitive.full.tiff", width=3500, height=1800, res=200)
...
for(k in 1:length(tags)){
	...
	which(all.gain.slim.mat[,1]==cancer & all.gain.slim.mat[,3] == drug & all.gain.slim.mat[, "gain.ps"] < 1e-8) -> ii
	if(length(ii) > 0){
...
which(all.gain.slim.mat$CHR == 8) -> idx
...
tiff("chr8.tiff", width=2000, height=1000, res=200)

plot(NA, xaxt='n', bty='n', xaxs='i', yaxs='i', las=1, pch=20, xlim=c(xmin,xmax), ylim=c(0,ceiling(max(d3$logp))), xlab="Chromosome 8", ylab=expression(-log[10](italic(p))))
...
for(k in 1:length(d3.sig.tags)){
	...
	with(d3[ii[imin], ], text(pos, logp, paste(d3.gain.slim.mat[ii[imin], 1:3], collapse="-"), pos=pos1, cex=.6, col=text.col ) )
...
tiff("chr17.tiff", width=2000, height=1000, res=200)
