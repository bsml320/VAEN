...
anno = read.table("/work/data/UCSC/", sep="\t", as.is=T)

###################################################################################################
dir.create("gain.full")
files = dir()
...
for(f in 1:length(files)){
	...
	write.table(cancer.gain.mat, file=paste("gain.full/",gsub("limma.mat", "limma.mat.full", files[f]), sep=""), row.names=F, col.names=F, quote=F, sep="\t" )
}

###################################################################################################
dir.create("loss.full")
files = dir()
...
for(f in 1:length(files)){
	...
	write.table(cancer.gain.mat, file=paste("loss.full/",gsub("limma.mat", "limma.mat.full", files[f]), sep=""), row.names=F, col.names=F, quote=F, sep="\t" )
}
