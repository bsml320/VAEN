...
heatmap.2(x1, trace="none", col=bluered(75) ) -> ccle.ori.h2
...
for(ct in 1:length(cancer.types)){
	...
	heatmap.2(as.matrix(blca.ccle[, 3:26]), trace="none", col=bluered(75), labRow="") -> ccle.h
}
dev.off()
...
heatmap.2(x1, trace="none", col=bluered(75), RowSideColors=rowBar) -> tcga.h2
...
plot(as.phylo(as.hclust(ccle.ori.h2$colDendrogram)), tip.color=drug.cc, direction="d", font=1, label.offset=0, cex=1, srt=-180, adj=1)
