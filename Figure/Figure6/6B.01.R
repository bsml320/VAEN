...
for(ct in 1:length(cancer.types)){
	...
	rm(sample2gene.length)
	rm(new.sample2gene.length)
}
write.table(cancer.drug.tml.mat, file="6B.CCLE.tml.ttest.txt", row.names=F, quote=F, sep="\t")
