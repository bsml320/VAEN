setwd("/work/RANK.Sigmoid/PCA/dr.CCLE/")
...
#####################################################################################
	...
	########### match
	save(model.list, file=paste("/work/RANK.Sigmoid/PCA/dr.CCLE/PCA.CCLE.model.list.RData", sep=""))
...
write.table(info, file="RANK.PCA.best.txt", row.names=F, col.names=T, quote=F, sep="\t")
...
write.table(TCGA.pred.mat, file="/work/RANK.Sigmoid/PCA/dr.CCLE/PCA.pred_TCGA.txt", quote=F, sep="\t", row.names=FALSE)

self.prediction.mat[,1] = rownames(Ys)
colnames(self.prediction.mat) = c("CELLLINE", drugs)
write.table(self.prediction.mat, file="/work/RANK.Sigmoid/PCA/dr.CCLE/PCA.pred_CCLE.txt", quote=F, sep="\t", row.names=FALSE)

self.pred.mat = cbind(CELLINE=rownames(CCLE.latent.data), CCLE.pred.full.mat)
colnames(self.pred.mat) = c("CELLINE", drugs)
write.table(self.pred.mat, file=paste("/work/RANK.Sigmoid/PCA/dr.CCLE/PCA.pred_CCLE.full.txt", sep=""), quote=F, sep="\t", row.names=FALSE)
